$(document).ready(function () {
	...
	var TableData = $("#DataTable").DataTable({
		...
		select: true,
		ajax: {
			url: $('meta[name="base_url"]').attr('content') + "FinanceAccounting/SaldoAwal/DT_list_saldoawal",
			dataType: "json",
			type: "POST",
			data: {
		...
		"buttons": [{
			text: `<i class="fas fa-plus fs-3"> Add Saldo Awal</i>`,
			className: "bg-primary",
			action: function (e, dt, node, config) {
				window.location.href = $('meta[name="base_url"]').attr('content') + "FinanceAccounting/SaldoAwal/add/"
			}
		}, {
	...
	function Init_Show_Detail(sysid) {
		window.location.href = `${$('meta[name="base_url"]').attr('content')}FinanceAccounting/SaldoAwal/edit/${sysid}`
	}
});
