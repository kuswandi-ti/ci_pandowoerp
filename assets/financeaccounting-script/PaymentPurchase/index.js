$(document).ready(function () {
	...
	function reloadData() {
		$("#DataTable").DataTable({
			...
			select: true,
			ajax: {
				url: $('meta[name="base_url"]').attr('content') + "FinanceAccounting/PaymentPurchase/DT_listdata",
				dataType: "json",
				type: "POST",
			},
			columns: [
			...
			"buttons": [{
				text: `<i class="fas fa-plus fs-3"> Add PP</i>`,
				...
				action: function (e, dt, node, config) {
					...
					if (RowData.length == 0) {
						...
						window.open($('meta[name="base_url"]').attr('content') + `FinanceAccounting/PaymentPurchase/print/${RowData[0].SysId}`, '_blank');
					}
				}
			}, {
	...
	function Init_Edit(SysId, State) {
		$.ajax({
			dataType: "json",
			type: "POST",
			url: $('meta[name="base_url"]').attr('content') + "FinanceAccounting/PaymentPurchase/edit",
			data: {
				sysid: SysId,
				state: State
			},
			success: function (response) {
				...
				if (response.code == 200) {
					...
					$('select[name="id_supplier"]').val(response.data_hdr.id_supplier).trigger('change');
					...
					$.each(response.data_dtl, function (index, rowData) {
						...
						$newRow.append('<td><input type="hidden" name="id_doc[]" value="' + rowData.id_doc + '"><p class="mt-1">' + no + '</p></td>');
						$newRow.append('<td><input class="form-control form-control-sm" name="no_doc[]" type="text" value="' + rowData.no_doc + '" readonly></td>');
						$newRow.append('<td><input class="form-control form-control-sm number" type="text" name="outstanding_payment[]" value="' + formatIdrAccounting(rowData.amount_payment) + '" readonly></td>');
						$newRow.append('<td><input class="form-control form-control-sm number" type="text" name="amount_payment[]" value="' + formatIdrAccounting(rowData.amount_payment) + '"></td>');
						$newRow.append('<td><input class="form-control form-control-sm" type="text" name="type_doc[]" value="' + rowData.type_doc + '" readonly></td>');
	...
	function reloadDataItem() {
		...
		var no_doc = [];
		var no_doc = $('input[name="no_doc[]"]').map(function () {
			return $(this).val();
		}).get();
		...
		$("#DataTable_Modal_ListItem").DataTable({
			...
			ajax: {
				url: $('meta[name="base_url"]').attr('content') + "FinanceAccounting/PaymentPurchase/DT_modallistofitem?no_doc=" + no_doc + "&id_supplier=" + id_supplier,
				dataType: "json",
				type: "POST",
			},
			columns: columnDefs,
		...
		$(document).on('click', '#select_item', function () {
			...
			$.each(selectedRows, function (no_doc, rowData) {
				if (rowData) {
			...
			$.each(selectedRowsData, function (index, rowData) {
				...
				$newRow.append('<td><input type="hidden" name="id_doc[]" value="' + rowData.id_doc + '"><p class="mt-1">' + no + '</p></td>');
				$newRow.append('<td><input class="form-control form-control-sm" name="no_doc[]" type="text" value="' + rowData.no_doc + '" readonly></td>');
				$newRow.append('<td><input class="form-control form-control-sm number" type="text" name="outstanding_payment[]" value="' + formatIdrAccounting(rowData.amount_os) + '" readonly></td>');
				$newRow.append('<td><input class="form-control form-control-sm number" type="text" name="amount_payment[]" value="' + formatIdrAccounting(rowData.amount_os) + '"></td>');
				$newRow.append('<td><input class="form-control form-control-sm" type="text" name="type_doc[]" value="' + rowData.type_doc + '" readonly></td>');
				$newRow.append('<td class="text-center"><a href="javascript:void(0);" class="remove_item_dtl"><span class="fa fa-times"></span></a></td>');
	...
	function Fn_Submit_Form() {
		...
		$.ajax({
			dataType: "json",
			type: "POST",
			url: $('meta[name="base_url"]').attr('content') + "FinanceAccounting/PaymentPurchase/store",
			data: formDataa,
			cache: false,
			contentType: false,
			processData: false,
	...
	function Fn_Toggle_Status(SysId) {
		Swal.fire({
			...
			if (result.isConfirmed) {
				$.ajax({
					url: $('meta[name="base_url"]').attr('content') + "FinanceAccounting/PaymentPurchase/Toggle_Status",
					type: "post",
					dataType: "json",
					data: {
						sysid: SysId,
						table: 'ttrx_hdr_payment_purchase'
					},
					beforeSend: function () {
						Swal.fire({
							title: 'Loading....',
	...
	function set_total() {
		var total = 0;
		$('input[name="amount_payment[]"]').map(function () {
	...
	$('body').on('keyup blur', $('input[name="amount_payment[]"]'), function () {
		set_total();
	});
});
