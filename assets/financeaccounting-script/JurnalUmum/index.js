$(document).ready(function () {
	...
	function reloadData() {
		$("#DataTable").DataTable({
			...
			select: true,
			ajax: {
				url: $('meta[name="base_url"]').attr('content') + "FinanceAccounting/JurnalUmum/DT_listdata",
				dataType: "json",
				type: "POST",
			},
			columns: [
			...
			"buttons": [{
				...
				action: function (e, dt, node, config) {
					...
					if (RowData.length == 0) {
						...
						window.open($('meta[name="base_url"]').attr('content') + `FinanceAccounting/JurnalUmum/print/${RowData[0].SysId}`, "_blank");
					}
				}
			}, {
	...
	function Init_Edit(SysId, State) {
		$.ajax({
			dataType: "json",
			type: "POST",
			url: $('meta[name="base_url"]').attr('content') + "FinanceAccounting/JurnalUmum/edit",
			data: {
				sysid: SysId,
                state: State
			},
	...
	function reloadDataItem() {
		...
		$("#DataTable_Modal_ListItem").DataTable({
			...
			ajax: {
				url: $('meta[name="base_url"]').attr('content') + "FinanceAccounting/JurnalUmum/DT_modallistofitem?id_coa=" + id_coa + "&id_akun_induk=" + id_akun_induk,
				dataType: "json",
				type: "POST",
			},
			columns: columnDefs,
	...
	function Fn_Submit_Form() {
		...
		$.ajax({
			dataType: "json",
			type: "POST",
			url: $('meta[name="base_url"]').attr('content') + "FinanceAccounting/JurnalUmum/store",
			data: formDataa,
			cache: false,
			contentType: false,
			processData: false,
