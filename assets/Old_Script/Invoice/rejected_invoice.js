$(document).ready(function () {
	...
	function Initialize_datatable() {
		$("#TableData").DataTable({
			...
			"responsive": true,
			ajax: {
				url: $('meta[name="base_url"]').attr('content') + "RejectedInvoice/DT_Rejected_Invoice",
				dataType: "json",
				type: "get",
				data: {
					from: $('#from').val(),
			...
			columns: [{
				...
				},
				{
					...
					render: function (data, type, row, meta) {
                        ...
                        <button data-toggle="tooltip" title="Detail Invoice" class="btn bg-gradient-primary btn-sm btn-detail" data-pk="${row.SysId}"><i class="fas fa-eye"></i></button></div>`
					}
				},
				{
					data: "Invoice_Number",
	...
	$(document).on('click', '.btn-detail', function () {
		...
		$.ajax({
			type: "GET",
			url: $('meta[name="base_url"]').attr('content') + "RejectedInvoice/M_Dtl_Invoice",
			data: {
				SysId_Invoice: SysId
			},
			beforeSend: function () {
