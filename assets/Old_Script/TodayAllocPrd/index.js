$(document).ready(function () {
	...
	var DataTable_Deskripsi = $("#DataTable_Deskripsi").DataTable({
		...
		ajax: {
			url: $('meta[name="base_url"]').attr('content') + "TodayAllocPrd/DataTable_alloc_prd_by_deskripsi",
			dataType: "json",
			type: "GET",
		},
		columns: [{
	...
	var DataTable_Lot = $("#DataTable_Lot").DataTable({
		...
		ajax: {
			url: $('meta[name="base_url"]').attr('content') + "TodayAllocPrd/DataTable_alloc_prd_by_lot",
			dataType: "json",
			type: "GET",
		},
		columns: [{
			...
			// },
			{
				...
				orderable: false,
				render: function (data, type, row, meta) {
					return `<small><b>${row.tgl_finish_sortir}</b></small>`
				}
			},
			{
				data: "qty",
			...
			},
			{
				...
				name: "time_alloc",
				render: function (data, type, row, meta) {
					return `<small class="badge badge-success">${row.time_alloc.substring(0, 16)}</small>`;
				}
			}, {
				data: 'product',
				name: "product"
	...
	$('#DataTable_Deskripsi tbody').on('click', 'button.btn-detail-lot', function () {
		...
		$.ajax({
			type: "GET",
			url: $('meta[name="base_url"]').attr('content') + "DatabaseLot/modal_list_lot_by_deskripsi_alloc_prd_today",
			data: {
				sysid_material: data['sysid_material'],
				status: 3,
				title: 'Material Alokasi Produksi'
			},
			beforeSend: function () {
				Swal.fire({
					title: 'Loading....',
