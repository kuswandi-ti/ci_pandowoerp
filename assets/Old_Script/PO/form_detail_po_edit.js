$(document).ready(function () {
	...
	$('#add-row').on('click', function () {
		...
		$.ajax({
			dataType: "json",
			type: "POST",
			url: $('meta[name="base_url"]').attr('content') + "InputPO/add_row_tmp_detail_po_edit",
			data: {
	...
	$('#remove-row').on('click', function () {
		...
		if (rowCount > 1) {
			$.ajax({
				dataType: "json",
				type: "POST",
				url: $('meta[name="base_url"]').attr('content') + "InputPO/delete_row_detail_po_edit",
				data: {
					sysid: sysid,
				},
				beforeSend: function () {
	...
	function Fn_Resgister_Po() {
		$.ajax({
			dataType: "json",
			type: "POST",
			url: $('meta[name="base_url"]').attr('content') + "InputPO/Store_Complete_Po_Edit",
			data: {
				No_Po_Internal: $('#No_Po_Internal').val(),
			},
			beforeSend: function () {
	...
	function Init_X_Editable_Qty() {
		$('.editable_qty').editable({
			...
			type: 'number',
			url: $('meta[name="base_url"]').attr('content') + "InputPO/store_editable_detail_qty_order_edit",
			title: 'Quantity...',
	...
	function Init_Select_Dua_Product() {
		$('select[name="Product[]"]').select2({
			...
			$.ajax({
				dataType: "json",
				type: "POST",
				url: $('meta[name="base_url"]').attr('content') + "InputPO/Get_utility_detail_product_edit",
				data: {
					sysid: sysid,
