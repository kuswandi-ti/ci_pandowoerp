    ...
    $('#submit--lotNo').on('click', function () {
        ...
        if (barcode.length < 9) {
            ...
            $.ajax({
                dataType: "json",
                type: "GET",
                url: $('meta[name="base_url"]').attr('content') + "BarcodeInOven/Insert_into_oven",
                data: {
                    barcode: barcode,
                    oven: oven,
                },
                ...
                success: function (response) {
                    ...
                    if (response.code == 200) {
                        Toast.fire({
                            icon: 'success',
                            title: 'Nomor lot ' + barcode + ' dinyatakan masuk oven!'
                        });
                        $('#no_barcode').val('');
        ...
        if (barcode == '') {
            ...
            if (barcode.length < 9) {
                ...
                $.ajax({
                    dataType: "json",
                    type: "GET",
                    url: $('meta[name="base_url"]').attr('content') + "BarcodeInOven/preview_detail_lot",
                    data: {
                        barcode: barcode
                    },
                    beforeSend: function () {
                    ...
                    success: function (response) {
                        ...
                        if (response.code == 200) {
                            ...
                            $("#FINISH").html(response.tgl_finish_sortir)
                            $("#STATUS").html(response.status)
                            $('#container-detail-barcode').show("slide", { direction: "left" }, 1000);
                        } else {
                            Swal.fire({
                                icon: 'error',
                                title: 'Oops...',
