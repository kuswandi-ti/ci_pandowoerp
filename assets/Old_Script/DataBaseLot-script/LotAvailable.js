$(document).ready(function () {
	...
	function Initialize_datatable() {
		$("#DataTable").DataTable({
			...
			ajax: {
				url: $('meta[name="base_url"]').attr('content') + "LotAvail/DataTable_Lot_Avail",
				dataType: "json",
				type: "GET",
				data: {
			...
			columns: [{
				...
				},
				{
					...
					render: function (data, type, row, meta) {
						return row.kode + '/<b>' + row.deskripsi + '</b>';
					}
				},
				{
					data: "harga_per_pcs",
				...
				},
				{
					...
					render: function (data, type, row, meta) {
						if (row.into_oven == '0') {
							...
							status_lot = `<span class="badge badge-danger">${row.status_kayu}</span>`
						...
						return status_lot + `&nbsp;&nbsp;<a href="${$('meta[name="base_url"]').attr('content')}DatabaseLpb/preview_detail_lpb/${row.lpb_hdr}" class="btn btn-xs bg-gradient-primary" data-toggle="tooltip" title="Detail LPB LOT"><i class="far fa-folder-open"></i></a>&nbsp;` +
							`<a href="${$('meta[name="base_url"]').attr('content')}DatabaseLpb/report_commercial_lpb/${row.lpb_hdr}" target="_blank" class="btn btn-xs bg-gradient-danger btn-print" data-pk="${row.sysid}" data-toggle="tooltip" title="Report Commercial LOT"><i class="fas fa-print"></i></a>`;
					}
				},
			],
			order: [
