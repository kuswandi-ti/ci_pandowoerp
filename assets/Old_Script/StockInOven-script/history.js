$(document).ready(function () {
	...
	function DataTable_Lot() {
		var DataTable_Lot = $("#DataTable_Lot").DataTable({
			...
			ajax: {
				url: $('meta[name="base_url"]').attr('content') + "HistoryOven/DataTable_HstOven_by_Lot",
				dataType: "json",
				type: "GET",
				data: {
					"from": $('#from').val(),
	...
	function DataTable_Desc() {
		$("#DataTable_Deskripsi").DataTable({
			...
			ajax: {
				url: $('meta[name="base_url"]').attr('content') + "HistoryOven/DataTable_HstOven_by_Desc",
				dataType: "json",
				type: "GET",
				data: {
					"from": $('#from').val(),
	...
	$('#DataTable_Deskripsi tbody').on('click', 'button.btn-detail-lot', function () {
		...
		$.ajax({
			type: "GET",
			url: $('meta[name="base_url"]').attr('content') + "HistoryOven/M_Hst_Lot_By_Material",
			data: {
				sysid_material: data['sysid_material'],
				status: 1,
				title: 'HISTORY MASUK OVEN',
				from: $('#from').val(),
				to: $('#to').val(),
				oven: $('#oven').find(":selected").val(),
				material: $('#material').find(":selected").val()
