$(document).ready(function () {
	...
	function DataTable_Lot() {
		var DataTable_Lot = $("#DataTable_Lot").DataTable({
			...
			ajax: {
				url: $('meta[name="base_url"]').attr('content') + "TrxWh/ExitKd/DataTable_HstOven_by_Lot",
				dataType: "json",
				type: "GET",
				data: {
					"from": $('#from').val(),
		...
		}).buttons().container().appendTo('#DataTable_Lot .col-md-6:eq(0)');
	}
	...
	function DataTable_Desc() {
		$("#DataTable_Deskripsi").DataTable({
			...
			ajax: {
				url: $('meta[name="base_url"]').attr('content') + "TrxWh/ExitKd/DataTable_HstOven_by_Desc",
				dataType: "json",
				type: "GET",
				data: {
					"from": $('#from').val(),
			...
			columns: [{
				...
				},
				{
					...
					orderable: false,
					render: function (data, type, row, meta) {
						return `<button class="btn btn-xs bg-gradient-info btn-detail-lot"><i class="fas fa-align-left"></i><i class="fas fa-align-right"></i></button>`
					}
				}
			],
			order: [
	...
	$('#DataTable_Deskripsi tbody').on('click', 'button.btn-detail-lot', function () {
		...
		$.ajax({
			type: "GET",
			url: $('meta[name="base_url"]').attr('content') + "TrxWh/ExitKd/M_Hst_Lot_By_Material",
			data: {
				sysid_material: data['sysid_material'],
				Id_Size_Item: data['Id_Size_Item'],
				status: 2,
				title: 'HISTORY KELUAR KILN',
				from: $('#from').val(),
				to: $('#to').val(),
				oven: $('#oven').find(":selected").val(),
				material: $('#material').find(":selected").val()
