$(document).ready(function () {
	...
	var TableData = $("#DataTable").DataTable({
		...
		select: true,
		ajax: {
			url: $('meta[name="base_url"]').attr('content') + "TrxWh/Adjustment/DT_List_Adjustment",
			dataType: "json",
			type: "POST",
		},
		columns: [{
		...
		"buttons": [{
			text: `<i class="fas fa-plus fs-3"></i> Add Penyesuaian`,
			className: "bg-primary",
			action: function (e, dt, node, config) {
				window.location.href = $('meta[name="base_url"]').attr('content') + "TrxWh/Adjustment/add"
			}
		}, {
			...
			action: function (e, dt, node, config) {
				...
				if (RowData.length == 0) {
					...
					window.location.href = `${$('meta[name="base_url"]').attr('content')}TrxWh/Adjustment/edit/${RowData[0].SysId}/update`
				}
			}
		}, {
			...
			action: function (e, dt, node, config) {
				...
				if (RowData.length == 0) {
					...
					window.location.href = `${$('meta[name="base_url"]').attr('content')}TrxWh/Adjustment/edit/${RowData[0].SysId}/preview`
				}
			}
		},
		// {
		...
		// 			});
		// 		} else {
		// 			window.open($('meta[name="base_url"]').attr('content') + `TrxWh/Adjustment/fn_print/${RowData[0].SysId}`, 'WindowReport-adjustment', 'width=800,height=600');
		// 		}
		// 	}
		// }, 
		{
	...
	function Fn_Cancel(SysId) {
		Swal.fire({
			...
			if (result.isConfirmed) {
				$.ajax({
					url: $('meta[name="base_url"]').attr('content') + "TrxWh/Adjustment/Cancel",
					type: "post",
					dataType: "json",
					data: {
						SysId: SysId
