$(document).ready(function () {
	...
	var TableData = $("#DataTable").DataTable({
		...
		select: true,
		ajax: {
			url: $('meta[name="base_url"]').attr('content') + "TrxWh/Adjustment/DT_List_Adjustment_Approval",
			dataType: "json",
			type: "POST",
		},
		columns: [{
	...
	function Fn_Toggle_Status(SysId, Param, Action_quote) {
		Swal.fire({
			...
			if (result.isConfirmed) {
				$.ajax({
					url: $('meta[name="base_url"]').attr('content') + "TrxWh/Adjustment/verify",
					type: "post",
					dataType: "json",
					data: {
						SysId: SysId,
	...
	function Init_Show_Detail(SysId) {
		window.location.href = `${$('meta[name="base_url"]').attr('content')}TrxWh/Adjustment/edit/${SysId}/preview`
	}
