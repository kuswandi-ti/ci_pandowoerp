$(document).ready(function () {
	...
	var TableData = $("#DataTable").DataTable({
		...
		select: true,
		ajax: {
			url: $('meta[name="base_url"]').attr('content') + "TrxWh/UsageNote/DT_List_Sun",
			dataType: "json",
			type: "POST",
		},
		columns: [{
		...
		"buttons": [{
			...
			action: function (e, dt, node, config) {
				window.location.href = $('meta[name="base_url"]').attr('content') + "TrxWh/UsageNote/add"
			}
		}, {
			...
			action: function (e, dt, node, config) {
				...
				if (RowData.length == 0) {
					...
					Fn_Initialize_form_edit(RowData[0].SysId)
				}
			}
		}, {
			...
			action: function (e, dt, node, config) {
				...
				if (RowData.length == 0) {
					...
					window.location.href = `${$('meta[name="base_url"]').attr('content')}TrxWh/UsageNote/edit/${RowData[0].SysId}/preview`
				}
			}
		}, {
	...
	function Fn_Cancel(SysId) {
		Swal.fire({
			...
			if (result.isConfirmed) {
				$.ajax({
					url: $('meta[name="base_url"]').attr('content') + "TrxWh/UsageNote/Cancel",
					type: "post",
					dataType: "json",
					data: {
						SysId: SysId
	...
	function Fn_Initialize_form_edit(SysId) {
		window.location.href = `${$('meta[name="base_url"]').attr('content')}TrxWh/UsageNote/edit/${SysId}/form`
	}
});
