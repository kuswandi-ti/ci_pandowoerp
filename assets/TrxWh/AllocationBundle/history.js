$(document).ready(function () {
	...
	function DataTable_Deskripsi() {
		var DataTable_Deskripsi = $("#DataTable_Deskripsi").DataTable({
			...
			ajax: {
				url: $('meta[name="base_url"]').attr('content') + "TrxWh/AllocationBundle/DataTable_alloc_prd_by_deskripsi",
				dataType: "json",
				type: "GET",
				data: {
					"from": $('#from').val(),
	...
	$('#DataTable_Deskripsi tbody').on('click', 'button.btn-detail-lot', function () {
		...
		$.ajax({
			type: "GET",
			url: $('meta[name="base_url"]').attr('content') + "TrxWh/AllocationBundle/modal_list_lot_by_deskripsi_alloc_prd",
			data: {
				sysid_material: data['sysid_material'],
				status: 3,
				title: 'Bundle Ter-Alokasi Ke Produksi',
				"from": $('#from').val(),
				"to": $('#to').val(),
				"product": $('#cc').find(":selected").val(),
