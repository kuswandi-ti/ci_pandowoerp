$(document).ready(function () {
	...
	function Initialize_datatable() {
		$("#DataTable").DataTable({
			...
			ajax: {
				url: $('meta[name="base_url"]').attr('content') + "TrxWh/Bundle/DT_Rekap_Material_Supplier",
				dataType: "json",
				type: "GET",
				data: {
					"from": $('#from').val(),
	...
	function Sum_Tot_Filter_Result() {
		$.ajax({
			type: "get",
			url: $('meta[name="base_url"]').attr('content') + "TrxWh/Bundle/Summary_Material_supplier",
			dataType: "json",
			data: {
				"from": $('#from').val(),
				"to": $('#to').val(),
			...
			success: function (response) {
				...
				if (response.code == 200) {
					...
					$('#tot-rupiah').text(response.rupiah)
					$('#footer').show()
				} else {
					Toast.fire({
						icon: 'error',
