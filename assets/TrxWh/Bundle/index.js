$(document).ready(function () {
	...
	function Initialize_datatable() {
		$("#DataTable").DataTable({
			...
			ajax: {
				url: $('meta[name="base_url"]').attr('content') + "TrxWh/Bundle/DT_Bundle_Lot",
				dataType: "json",
				type: "GET",
				data: {
					"from": $('#from').val(),
			...
			columns: [{
					...
					render: function (data, type, row, meta) {
						return row.lpb_hdr + `<br/><a href="${$('meta[name="base_url"]').attr('content')}TrxWh/Lpb/preview_detail_lpb/${row.lpb_hdr}" class="btn btn-xs bg-gradient-primary" data-toggle="tooltip" title="Detail LPB"><i class="far fa-folder-open"></i></a>&nbsp;` +
							`<a href="${$('meta[name="base_url"]').attr('content')}TrxWh/Lpb/report_commercial_lpb/${row.lpb_hdr}" target="_blank" class="btn btn-xs bg-gradient-danger btn-print" data-pk="${row.sysid}" data-toggle="tooltip" title="Report Commercial LPB"><i class="fas fa-print"></i></a>`;
					}
				},
				{
					data: "supplier",
	...
	$('#DataTable tbody').on('click', 'a.detail--size', function () {
