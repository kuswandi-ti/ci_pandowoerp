$(document).ready(function () {
	...
	var DataTable_Deskripsi = $("#DataTable_Deskripsi").DataTable({
		...
		ajax: {
			url: $('meta[name="base_url"]').attr('content') + "TrxWh/StokBasah/DataTable_Stock_Kayu_Basah_by_deskripsi",
			dataType: "json",
			type: "GET",
		},
		columns: [{
	...
	var DataTable_Lot = $("#DataTable_Lot").DataTable({
		...
		ajax: {
			url: $('meta[name="base_url"]').attr('content') + "TrxWh/StokBasah/DataTable_Stock_Kayu_Basah_by_lot",
			dataType: "json",
			type: "GET",
		},
		columns: [{
		...
		},
		{
			...
			orderable: false,
			render: function (data, type, row, meta) {
				return `<button class="btn btn-xs bg-gradient-danger btn-into-oven" title="into oven!"> &nbsp; <i class="fas fa-download"></i> Masuk Kiln!</button>`
			}
		}
		],
		order: [
	...
	$('#DataTable_Deskripsi tbody').on('click', 'button.btn-detail-lot', function () {
		...
		$.ajax({
			type: "GET",
			url: $('meta[name="base_url"]').attr('content') + "TrxWh/StokBasah/modal_list_lot_by_deskripsi",
			data: {
				sysid_material: data['sysid_material'],
				Id_Size_Item: data['Id_Size_Item'],
				status: 0,
				title: 'STOK KAYU BASAH'
			},
			beforeSend: function () {
				Swal.fire({
					title: 'Loading....',
	...
	let inputOptionsPromise = new Promise(function (resolve) {
		$.getJSON($('meta[name="base_url"]').attr('content') + "TrxWh/Bundle/List_Kd_active", function (data) {
			resolve(data)
		});
	})
