$(document).ready(function () {
	...
	var TableData = $("#DataTable").DataTable({
		...
		select: true,
		ajax: {
			url: $('meta[name="base_url"]').attr('content') + "TrxWh/LpbAfkir/DT_List_Lpb_Afkir",
			dataType: "json",
			type: "POST",
		},
		columns: [{
		...
		"buttons": [{
			...
			action: function (e, dt, node, config) {
				window.location.href = $('meta[name="base_url"]').attr('content') + "TrxWh/LpbAfkir/add"
			}
		}, {
			...
			action: function (e, dt, node, config) {
				...
				if (RowData.length == 0) {
					...
					window.location.href = `${$('meta[name="base_url"]').attr('content')}TrxWh/LpbAfkir/edit/${RowData[0].SysId}/update`
				}
			}
		}, {
			...
			action: function (e, dt, node, config) {
				...
				if (RowData.length == 0) {
					...
					window.location.href = `${$('meta[name="base_url"]').attr('content')}TrxWh/LpbAfkir/edit/${RowData[0].SysId}/preview`
				}
			}
		}, {
	...
	function Fn_Cancel(SysId) {
		Swal.fire({
			...
			if (result.isConfirmed) {
				$.ajax({
					url: $('meta[name="base_url"]').attr('content') + "TrxWh/LpbAfkir/Cancel",
					type: "post",
					dataType: "json",
					data: {
						SysId: SysId
