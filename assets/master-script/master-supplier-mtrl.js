$(document).ready(function () {
	$('#tbl-supplier-mtrl thead tr')
		.clone(true)
		.addClass('filters')
		.appendTo('#tbl-supplier-mtrl thead');
	var table = $("#tbl-supplier-mtrl").DataTable({
		"responsive": true,
		"lengthChange": true,
		"autoWidth": true,
		"select": true,
	...
	}).buttons().container().appendTo('#tbl-supplier-mtrl_wrapper .col-md-6:eq(0)');
	$('.select2').select2({
		tags: true,
		placeholder: "-Filter-",
		allowClear: true
	...
	$(document).on('click', '.is-active', function () {
		...
		Swal.fire({
			...
			text: `Apakah anda yakin untuk merubah status supplier ini ?`,
			icon: 'warning',
			...
			if (result.isConfirmed) {
				$.ajax({
					dataType: "json",
					type: "POST",
					url: $('meta[name="base_url"]').attr('content') + "Master/toggle_status_supplier",
					data: {
	...
	$(document).on('click', '.btn-edit', function () {
		$.ajax({
			type: "GET",
			url: $('meta[name="base_url"]').attr('content') + "Master/modal_edit_suppplier",
			data: {
				sysid: $(this).attr('data-pk')
			},
			beforeSend: function () {
	...
	$(document).on('click', '.btn-manage', function () {
		...
		$.ajax({
			type: "GET",
			url: $('meta[name="base_url"]').attr('content') + "Master/modal_manage_harga",
			data: {
				sysid: $(this).attr('data-pk')
			},
			beforeSend: function () {
	...
	$(document).on('click', '.btn-history', function () {
		...
		$.ajax({
			type: "GET",
			url: $('meta[name="base_url"]').attr('content') + "Master/modal_history_harga",
			data: {
				sysid: $(this).attr('data-pk')
			},
			beforeSend: function () {
