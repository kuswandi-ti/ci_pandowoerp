$(document).ready(function () {
	...
	var TableData = $("#DataTable").DataTable({
		...
		select: true,
		ajax: {
			url: $('meta[name="base_url"]').attr('content') + "MasterData/CostCenter/DT_list_costcenter",
			dataType: "json",
			type: "POST",
			data: {
		...
		"buttons": [{
			text: `<i class="fas fa-plus fs-3"> Add Cost Center</i>`,
			className: "bg-primary",
			action: function (e, dt, node, config) {
				window.location.href = $('meta[name="base_url"]').attr('content') + "MasterData/CostCenter/add/"
			}
		}, 
		{
