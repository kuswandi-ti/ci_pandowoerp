$(document).ready(function () {
    ...
    var TableData = $("#DataTable").DataTable({
        ...
        select: true,
        ajax: {
            url: $('meta[name="base_url"]').attr('content') + "MasterData/Tax/DT_tax",
            dataType: "json",
            type: "POST",
        },
        columns: [
        ...
        "buttons": [{
            text: `<i class="fas fa-plus fs-3"></i> Add Jenis Pajak`,
            className: "bg-primary",
            action: function (e, dt, node, config) {
                window.location.href = $('meta[name="base_url"]').attr('content') + "MasterData/Tax/add"
            }
        }, {
            ...
            action: function (e, dt, node, config) {
                ...
                if (RowData.length == 0) {
                    ...
                    Fn_Toggle_Status(RowData[0].Tax_Id)
                }
            }
        }, {
            text: `Export to :`,
    ...
    function Fn_Toggle_Status(Tax_Id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: $('meta[name="base_url"]').attr('content') + "MasterData/Tax/Toggle_Status",
                    type: "post",
                    dataType: "json",
                    data: {
                        Tax_Id: Tax_Id
