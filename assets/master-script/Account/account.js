$(document).ready(function () {
	...
	var TableData = $("#DataTable").DataTable({
		...
		select: true,
		ajax: {
			url: $('meta[name="base_url"]').attr('content') + "MasterData/Account/DT_list_account",
			dataType: "json",
			type: "POST",
			data: {
		...
		"buttons": [{
			text: `<i class="fas fa-plus fs-3"></i> Add New`,
			className: "bg-primary",
			action: function (e, dt, node, config) {
				window.location.href = $('meta[name="base_url"]').attr('content') + "MasterData/Account/add/" + $('#IdentityPattern').val()
			}
		}, {
