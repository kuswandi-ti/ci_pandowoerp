$(document).ready(function () {
	...
	var TableData = $("#DataTable").DataTable({
		...
		select: true,
		ajax: {
			url: $('meta[name="base_url"]').attr('content') + "MasterData/Periode/DT_list_periode",
			dataType: "json",
			type: "POST",
			data: {
		...
		columns: [
			...
			},
			{
				...
				render: function (data, type, row, meta) {
						if (data == 1) {
							return `<div class='d-flex justify-content-center'><span class="badge bg-success">Open</span></div>`;
						} else {
							return `<div class='d-flex justify-content-center'><span class="badge bg-danger">Close</span></div>`;
						}
					}
				// render: function (data, type, row, meta) {
				// 	if (data == 1) {
		...
		"buttons": [{
			text: `<i class="fas fa-plus fs-3"> Add Periode</i>`,
			className: "bg-primary",
			action: function (e, dt, node, config) {
				window.location.href = $('meta[name="base_url"]').attr('content') + "MasterData/Periode/add/"
			}
		}, 
		// {
		...
		// }, 
		{
			text: `<i class="fas fa-toggle-on"> Open / Close</i>`,
	...
	function Init_Show_Detail(sysid) {
		window.location.href = `${$('meta[name="base_url"]').attr('content')}MasterData/AkunInduk/edit/${sysid}`
	}
});
