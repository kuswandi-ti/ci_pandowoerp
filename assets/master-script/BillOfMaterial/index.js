$(document).ready(function () {
	...
	var TableData = $("#DataTable").DataTable({
		...
		select: true,
		ajax: {
			url: $('meta[name="base_url"]').attr('content') + "MasterData/BillOfMaterial/DT_list_BOM",
			dataType: "json",
			type: "POST",
			data: {
		...
		"buttons": [{
			text: `<i class="fas fa-plus fs-3"> Add BOM</i>`,
			className: "bg-primary",
			action: function (e, dt, node, config) {
				window.location.href = $('meta[name="base_url"]').attr('content') + "MasterData/BillOfMaterial/add/"
			}
		}, 
		{
	...
	function Init_Show_Detail(sysid) {
		window.location.href = `${$('meta[name="base_url"]').attr('content')}MasterData/BillOfMaterial/edit/${sysid}`
	}
