$(document).ready(function () {
	...
	jQuery('#no_polisi').keyup(function () {
	...
	$(document).on('click', '.is-active', function () {
		...
		Swal.fire({
			...
			text: `Apakah anda yakin untuk merubah status kendaraan ini ?`,
			icon: 'warning',
			...
			if (result.isConfirmed) {
				$.ajax({
					dataType: "json",
					type: "POST",
					url: $('meta[name="base_url"]').attr('content') + "Master/toggle_status_kendaraan",
					data: {
