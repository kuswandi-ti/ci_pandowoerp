$(document).ready(function () {
	...
	var TableData = $("#DataTable").DataTable({
		...
		select: true,
		ajax: {
			url: $('meta[name="base_url"]').attr('content') + "MasterData/TipeAkun/DT_list_tipeakun",
			dataType: "json",
			type: "POST",
			data: {
		...
		"buttons": [{
			text: `<i class="fas fa-plus fs-3"> Add Tipe Akun</i>`,
			className: "bg-primary",
			action: function (e, dt, node, config) {
				window.location.href = $('meta[name="base_url"]').attr('content') + "MasterData/TipeAkun/add/"
			}
		}, 
		// {
	...
	function Init_Show_Detail(sysid) {
		window.location.href = `${$('meta[name="base_url"]').attr('content')}MasterData/TipeAkun/edit/${sysid}`
	}
});
