$(document).ready(function () {
	...
	var TableData = $("#DataTable").DataTable({
		...
		select: true,
		ajax: {
			url: $('meta[name="base_url"]').attr('content') + "MasterData/PohonIndustri/DT_list_pohon_industri",
			dataType: "json",
			type: "POST",
		},
		columns: [{
			...
			{
				data: "Nama_Pohon_Kayu",
				name: "Nama_Pohon_Kayu",
			},
			{
				data: "Grouping_Code",
				name: "Grouping_Code",
		...
		"buttons": [{
				...
				action: function (e, dt, node, config) {
					window.location.href = $('meta[name="base_url"]').attr('content') + "MasterData/PohonIndustri/add"
				}
			},
			// {
		...
		Swal.fire({
			...
			if (result.isConfirmed) {
				$.ajax({
					...
					data: {
						sysid: SysId,
						table: 'tmst_pohon_kayu_industri'
					},
					beforeSend: function () {
						Swal.fire({
							title: 'Loading....',
	...
	function Init_Show_Detail(sysid) {
		window.location.href = `${$('meta[name="base_url"]').attr('content')}MasterData/CostCenter/edit/${sysid}`
	}
});
