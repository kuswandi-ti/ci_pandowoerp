...
class LabaRugi extends CI_Controller
{
	...
	public function index()
	{
		...
		$data['page_title'] = "List of Laba Rugi";
		$data['page_content'] = "FinanceAccounting/LabaRugi/index";
		...
		$data['script_page'] =  '<script src="' . base_url() . 'assets/financeaccounting-script/LabaRugi/index.js?v=' . time() . '"></script>';
	...
	{
		$hasil = "";
		$leftrightsection = ($section == "left") ? "3" : "4";

		$total_all = 0;

		if ($leftrightsection == '3') {
			...
			// 	"(SUM(COALESCE(C.credit, 0)) - SUM(COALESCE(C.debit, 0))) + SUM(COALESCE(D.debit, 0)) - SUM(COALESCE(D.credit, 0)) AS saldo_akhir " .
			...
			// 	"WHERE A.kode_tipe_akun IN (" . $leftrightsection . ") AND (D.debit > 0 OR D.credit > 0) AND (E.tgl_jurnal BETWEEN '" . $dari_tanggal . "' AND '" . $sampai_tanggal . "') " .
			...
			$qry_tipe_akun = $this->db->query("SELECT A.SysId, CONCAT_WS(' - ', A.kode_tipe_akun, A.nama_tipe_akun) AS akun, " .
				...
				"LEFT OUTER JOIN ttrx_dtl_jurnal D ON C.SysId = D.id_coa " .
				"LEFT OUTER JOIN ttrx_hdr_jurnal E ON D.id_hdr = E.SysId " .
				"WHERE A.kode_tipe_akun IN (" . $leftrightsection . ") AND (D.debit > 0 OR D.credit > 0) AND (E.tgl_jurnal BETWEEN '" . $dari_tanggal . "' AND '" . $sampai_tanggal . "') AND E.isCancel = 0 " .
				"GROUP BY A.SysId, A.kode_tipe_akun, A.nama_tipe_akun");
		} else {
			$qry_tipe_akun = $this->db->query("SELECT A.SysId, CONCAT_WS(' - ', A.kode_tipe_akun, A.nama_tipe_akun) AS akun, " .
				"SUM(COALESCE(C.debit, 0)) - SUM(COALESCE(C.credit, 0)) AS saldo_awal, " .
				"SUM(COALESCE(D.debit, 0)) AS debit, SUM(COALESCE(D.credit, 0)) AS credit, " .
				"(SUM(COALESCE(C.debit, 0)) - SUM(COALESCE(C.credit, 0))) + SUM(COALESCE(D.credit, 0)) - SUM(COALESCE(D.debit, 0)) AS saldo_akhir " .
				"FROM tmst_akun_tipe A " .
				"LEFT OUTER JOIN tmst_akun_induk B ON A.SysId = B.id_akun_tipe " .
				"LEFT OUTER JOIN tmst_chart_of_account C ON B.SysId = C.id_akun_induk " .
				"LEFT OUTER JOIN ttrx_dtl_jurnal D ON C.SysId = D.id_coa " .
		...
		if (($qry_tipe_akun->num_rows()) > 0) {
			foreach ($qry_tipe_akun->result() as $row1) {
				...
				if ($leftrightsection == '3') {
					...
					// 	"(SUM(COALESCE(B.credit, 0)) - SUM(COALESCE(B.debit, 0))) + SUM(COALESCE(C.debit, 0)) - SUM(COALESCE(C.credit, 0)) AS saldo_akhir " .
					...
					$qry_induk_akun = $this->db->query("SELECT A.SysId, CONCAT_WS(' - ', A.kode_akun, A.nama_akun) AS akun, " .
						...
						"WHERE A.id_akun_tipe = '" . $row1->SysId . "' AND (C.debit > 0 OR C.credit > 0) AND (D.tgl_jurnal BETWEEN '" . $dari_tanggal . "' AND '" . $sampai_tanggal . "') AND D.isCancel = 0 " .
						"GROUP BY A.SysId, A.kode_akun, A.nama_akun");
				} else {
					$qry_induk_akun = $this->db->query("SELECT A.SysId, CONCAT_WS(' - ', A.kode_akun, A.nama_akun) AS akun, " .
						"SUM(COALESCE(B.debit, 0)) - SUM(COALESCE(B.credit, 0)) AS saldo_awal, " .
						"SUM(COALESCE(C.debit, 0)) AS debit, SUM(COALESCE(C.credit, 0)) AS credit, " .
						"(SUM(COALESCE(B.debit, 0)) - SUM(COALESCE(B.credit, 0))) + SUM(COALESCE(C.credit, 0)) - SUM(COALESCE(C.debit, 0)) AS saldo_akhir " .
						"FROM tmst_akun_induk A " .
						"LEFT OUTER JOIN tmst_chart_of_account B ON A.SysId = B.id_akun_induk " .
						"LEFT OUTER JOIN ttrx_dtl_jurnal C ON B.SysId = C.id_coa " .
						"LEFT OUTER JOIN ttrx_hdr_jurnal D ON C.id_hdr = D.SysId " .
				...
				if (($qry_induk_akun->num_rows()) > 0) {
					foreach ($qry_induk_akun->result() as $row2) {
						...
						if ($leftrightsection == '3') {
							...
							// 	"(SUM(COALESCE(A.credit, 0)) - SUM(COALESCE(A.debit, 0))) + SUM(COALESCE(B.debit, 0)) - SUM(COALESCE(B.credit, 0)) AS saldo_akhir " .
							...
							$qry_coa = $this->db->query("SELECT CONCAT_WS(' - ', A.kode_akun, A.nama_akun) AS akun, SUM(COALESCE(A.debit, 0)) - SUM(COALESCE(A.credit, 0)) AS saldo_awal, " .
								...
								"WHERE A.id_akun_induk = '" . $row2->SysId . "' AND (B.debit > 0 OR B.credit > 0) AND (C.tgl_jurnal BETWEEN '" . $dari_tanggal . "' AND '" . $sampai_tanggal . "') AND C.isCancel = 0 " .
								"GROUP BY A.kode_akun, A.nama_akun");
						} else {
							$qry_coa = $this->db->query("SELECT CONCAT_WS(' - ', A.kode_akun, A.nama_akun) AS akun, SUM(COALESCE(A.debit, 0)) - SUM(COALESCE(A.credit, 0)) AS saldo_awal, " .
								"SUM(COALESCE(B.debit, 0)) AS debit, SUM(COALESCE(B.credit, 0)) AS credit, " .
	...
	public function DT_List_LabaRugi()
	{
