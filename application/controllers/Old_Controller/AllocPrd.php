...
class AllocPrd extends CI_Controller
{
    ...
    {
        $this->data['page_title'] = "Alokasi Material Produksi";
        $this->data['page_content'] = "AllocPrd/index";
        $this->data['script_page'] =  '<script src="' . base_url() . 'assets/AllocPrd/index.js"></script>';

        $this->data['products'] = $this->db->get_where(
            'tmst_hdr_product',
            ['is_active' => '1']
        )->result();
    ...
    public function DataTable_alloc_prd_by_deskripsi()
    {
        ...
        if (!empty($product)) {
            $filter_product = " AND d.sysid_product = $product ";
        } else {
            $filter_product = "";
        }
    ...
    public function DataTable_alloc_prd_by_lot()
    {
        ...
        $sql = "SELECT a.no_lot, d.nama , b.deskripsi, b.kode, c.grader, e.do_time as time_in, f.do_time as time_out, g.do_time as time_alloc,
         c.tgl_kirim, c.tgl_finish_sortir, a.qty, a.sysid_material, a.qty * ((b.tebal * b.lebar * b.panjang) / 1000000) as kubikasi, h.nama as nama_oven,i.Nama as nama_product,
        ...
        foreach ($query->result_array() as $row) {
            ...
            $nestedData['nama_product'] = $row["nama_product"];
